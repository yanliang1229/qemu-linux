...
/**
 * 请求队列: make_request_fn()bio 处理接口
 * 通用块层的BIO请求，转化为块设备层的request请求
 */
static blk_qc_t blk_queue_bio(struct request_queue *q, struct bio *bio)
{
...
get_rq:
	 ...
	 * 通用块层BIO不能进行合并，从块设备层驱动层获取一个空闲req
	 */
...
generic_make_request_checks(struct bio *bio)
{
	 ...
	 * 如果bio是操作磁盘分区的，转化磁盘逻辑扇区号为整个磁盘的绝对扇区号
	 */
...
blk_qc_t generic_make_request(struct bio *bio)
{
	...
	do {
		...
		if (likely(blk_queue_enter(q, __GFP_DIRECT_RECLAIM) == 0)) {
			...
			blk_queue_exit(q);
			/**
			 * 执行bio_list_pop函数再次获取bio_list中的bio，因此可以将
			 * 发生递归调用generic_make_request()添加的bio结构拿出来进行处理
			 */
