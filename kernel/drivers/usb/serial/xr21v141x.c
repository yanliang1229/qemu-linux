...
int xr_usb_serial_set_control(struct xr_usb_serial *xr_usb_serial, unsigned int control)
{
	...
	if (control & XR_USB_SERIAL_CTRL_DTR)
		ret += xr_usb_serial_set_reg(xr_usb_serial,xr_usb_serial->reg_map.uart_gpio_clr_addr, 0x08);
	else
		ret += xr_usb_serial_set_reg(xr_usb_serial,xr_usb_serial->reg_map.uart_gpio_set_addr, 0x08);

	if (control & XR_USB_SERIAL_CTRL_RTS)
		ret += xr_usb_serial_set_reg(xr_usb_serial,xr_usb_serial->reg_map.uart_gpio_clr_addr, 0x20);
	else
		ret += xr_usb_serial_set_reg(xr_usb_serial,xr_usb_serial->reg_map.uart_gpio_set_addr, 0x20);
...
int xr_usb_serial_set_line(struct xr_usb_serial *xr_usb_serial, struct usb_cdc_line_coding* line)
{
	...
	ret = xr21v141x_set_baud_rate(xr_usb_serial, le32_to_cpu(line->dwDTERate));
	format_size = line->bDataBits;
	format_parity = line->bParityType;
	format_stop = line->bCharFormat;
	ret += xr_usb_serial_set_reg(xr_usb_serial,
...
void xr_usb_serial_set_wide_mode(struct xr_usb_serial *xr_usb_serial, int preciseflags)
{
	int channel = xr_usb_serial->channel;
	xr_usb_serial_disable(xr_usb_serial);
	if(channel)
...
void xr_usb_serial_pre_setup(struct xr_usb_serial *xr_usb_serial)
{
	...
	xr_usb_serial_set_reg(xr_usb_serial, xr_usb_serial->reg_map.uart_gpio_dir_addr, 0x20);
	xr_usb_serial_set_reg(xr_usb_serial, xr_usb_serial->reg_map.uart_gpio_clr_addr, UART_GPIO_CLR_RTS);
	xr_usb_serial_set_reg(xr_usb_serial, xr_usb_serial->reg_map.uart_gpio_set_addr, UART_GPIO_SET_DTR);
}
...
static int xr_usb_serial_start_wb(struct xr_usb_serial *xr_usb_serial, struct xr_usb_serial_wb *wb)
{
	...
	dev_err(&xr_usb_serial->data->dev, "%s\n", __func__);
	xr_usb_serial->transmitting++;
...
static void xr_usb_serial_write_bulk(struct urb *urb)
{
	...
	unsigned long flags;
	dev_vdbg(&xr_usb_serial->data->dev, "%s status : %d\n", __func__, urb->status);
...
static int xr_usb_serial_port_activate(struct tty_port *port, struct tty_struct *tty)
{
	...
	xr_usb_serial->ctrlout &= ~XR_USB_SERIAL_CTRL_RTS;
	xr_usb_serial->ctrlout |= XR_USB_SERIAL_CTRL_DTR;
