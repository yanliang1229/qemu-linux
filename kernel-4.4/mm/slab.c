 ...
 */
struct array_cache {
	unsigned int avail;	/* entry数组中缓存的slab对象的个数 */
	unsigned int limit;	/* entry数组的大小 */
			...
			size_t size, size_t align, unsigned long flags)
{
	...
	for (gfporder = 0; gfporder <= KMALLOC_MAX_ORDER; gfporder++) {
		 ...
		 * 单个SLAB分配管理的slab对象个数不超过SLAB_OBJ_MAX_NUM
		 */
...
#endif
	/* 释放的对象，重新放入slab对象区域, 放入寻找索引有点类似LRU */
	page->active--;
