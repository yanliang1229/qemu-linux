	...
	int (*fn)(struct snd_soc_dapm_widget *, struct list_head *))
{
	...
	snd_soc_dapm_widget_for_each_path(widget, rdir, path) {
		...
		/* path路径处于连接状态，继续遍历，找到endpoint w */
		if (path->connect) {
			path->walking = 1;
 ...
 * widget一路往后走，直至达到output endpoint，统计widget到output endpoints的路径数目
 *
 * output endpoint类型:
 *  1. snd_soc_dapm_adc
 *  2. snd_soc_dapm_aif_out
 *  3. snd_soc_dapm_output
 *  4. snd_soc_dapm_hp
 *  5. snd_soc_dapm_spk
 */
 ...
 * widget连接到input endpoint的路径数目
 *
 * input endpoint:
 *  1. snd_soc_dapm_dac
 *  2. snd_soc_dapm_aif_in
...
static int dapm_generic_check_power(struct snd_soc_dapm_widget *w)
{
	...
	/**
	 * 如果连接到output endpoint的路径数目为0或者连接到input endpoint的路径数目为0，
	 * 那么说明该widget没有构成任意合法的完整的音频路径，因此也就应该关闭该widget
	 */
			 ...
			 struct snd_soc_dapm_widget *sink))
{
	...
	/**
	 * 该条path路径加入到输入输出两个方向的链表中:
	 * 	list_add(&path->list_node[0], wsource->edges[0]);
	 * 	list_add(&path->list_node[1], wsink->edges[1]);
	 */
	snd_soc_dapm_for_each_direction(dir)
			 ...
			 const struct snd_soc_dapm_widget *widget)
{
	...
	/* 将dapm widget，加入到card中的widgets管理链表中 */
 ...
 *
 * Returns 0 for success else error.
 * 创建dapm窗口，并加入到card中的widget管理链表中
 */
